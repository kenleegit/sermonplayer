 ...
 * @since  1.0
 */
class PlgContentSermonplayer extends JPlugin
{
	/**
	...
	public function onContentPrepare($context, &$article, &$params, $page = 0)
	{
		...
		if (strpos($article->text, 'sermonplayer') === false)
		{
			return;
		}

		// Expression to search for (positions)
		$regex = '/{sermonplayer\s+(.*?)}/i';
		...
		if ($matches)
		{
			...
			foreach ($matches as $i => $match)
			{
				...
				//$item->audiofile = 'https://s3.eu-central-1.amazonaws.com/mp3-test-files/sample.mp3';
				$item->audiofile = $url;
